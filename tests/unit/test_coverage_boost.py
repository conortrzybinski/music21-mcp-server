...
class TestExtraCoverage:
    ...
    def test_retry_logic_extra_coverage(self):
        ...
        # Test should retry
        assert policy.should_retry(RetryableError("test"), 1)
        assert not policy.should_retry(ValueError("test"), 1)
        assert not policy.should_retry(RetryableError("test"), 5)
...
class TestStyleImitationToolComprehensive:
    ...
    def test_stylistic_pitch_generation(self, style_tool):
        ...
        style_data = {"melodic": {"stepwise_motion": 0.8, "avg_interval": 3.0}}
    ...
    def test_stylistic_duration_generation(self, style_tool):
        ...
        style_data = {"rhythmic": {"avg_duration": 1.0}}
    ...
    def test_bass_line_generation(self, style_tool):
        ...
        for bass_note, melody_note in zip(bass_notes, melody_notes, strict=False):
...
class TestAsyncOptimizationComprehensive:
    ...
    async def test_progression_detection_fast(self, async_optimizer):
        ...
        found_progression = any(prog["name"] == "I-V-vi-IV" for prog in progressions)
        assert found_progression

        # Test with no matching progressions
        random_progression = ["vii°", "III", "vi", "ii"]
        progressions2 = await async_optimizer.detect_progressions_fast(
            random_progression
        )
    ...
    async def test_warm_caches(self, async_optimizer):
        ...
        custom_progressions = [["C", "Am", "F", "G"], ["Dm", "G", "C", "C"]]
...
class TestPatternRecognitionToolComprehensive:
    ...
    def test_validate_inputs(self, pattern_tool):
        ...
        error = pattern_tool.validate_inputs(
            score_id="test_score", pattern_type="invalid"
        )
        assert "Invalid pattern_type" in error

        # Test invalid similarity threshold
        error = pattern_tool.validate_inputs(
            score_id="test_score", similarity_threshold=1.5
        )
        assert "similarity_threshold must be between 0 and 1" in error

        error = pattern_tool.validate_inputs(
            score_id="test_score", similarity_threshold=-0.5
        )
        assert "similarity_threshold must be between 0 and 1" in error

        # Valid inputs
        error = pattern_tool.validate_inputs(
            score_id="test_score", pattern_type="melodic", similarity_threshold=0.8
        )
    ...
    def test_classify_contour_shape(self, pattern_tool):
        ...
        assert shape in [
            ...
            "undulating",
            "static",
        ]  # Could be various depending on calculation
